...
import { IconPinOutline16, IconPanelRight } from './icons.tsx'
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  // The DSH layout auto-collapses the left sessions panel below half the
  // physical screen width (see dsh-client-ui-layout sidebarAutoCollapseWidth).
  // Mirror that rule here so opening the right panel on a <50%-screen window
  // can force-close the left panel to 0px.
  const belowHalfScreen = typeof window !== 'undefined'
  ...
  /** The surface the rail highlights: the active tab's type in the right
   *  workbench. Falls back to the right tree's first populated leaf so a
   *  stale activePane (which may point into the bottom tree) never blanks it. */
  const activeSurfaceId = useMemo(() => {
    if (state === undefined) return undefined
    const leaves = allLeaves(state.splits)
    const leaf = (state.activePane !== null && state.activePane !== ''
      ? leaves.find((l) => l.id === state.activePane)
      : undefined) ?? leaves.find((l) => l.active !== null)
    const tab = leaf?.tabs.find((t) => t.id === leaf.active)
    return tab?.type
  }, [state])

  /** Rail activation: focus the surface's open tab (floats raise) or open a
   *  fresh one when the surface is idle. Mirrors onNewTab's title resolution. */
  const activateSurface = useCallback((surfaceId: string): void => {
    const service = ctx.get('betterSidebar')
    if (service === undefined || state === undefined || sessionId === undefined) return
    const scope = { sessionId, cwd }
    const open = allLeaves(state.splits)
      .concat(allLeaves(state.bottomSplits))
      .flatMap((leaf) => leaf.tabs)
      .find((tab) => tab.type === surfaceId)
      ?? state.floats.find((f) => f.tab.type === surfaceId)?.tab
    if (open !== undefined) { service.activateTab(open.id, scope); return }
    const descriptor = service.getTab(surfaceId)
    if (descriptor === undefined) return
    const title = typeof descriptor.title === 'function' ? descriptor.title() : descriptor.title
    service.openTab({ type: surfaceId, title }, scope)
  }, [ctx, state, sessionId, cwd])
  ...
  // Opening the right panel on a <50%-screen window also force-closes the
  // left sessions panel to 0px, so the conversation gets the full width.
  const handleTogglePanel = (): void => {
    const opening = state?.panelOpen !== true
    store.reduce(togglePanel)
    if (opening && belowHalfScreen) ctx.get('layout')?.closeSidebar()
  }
  ...
  return (
    <div data-dsh-panel-host {...osFileDragShield}>
      {/*
        The right-panel toggle lives OUTSIDE the panel (floating) so the
        panel can be opened from the main GUI when it is CLOSED. When it is
        OPEN, this floating icon hides and the rail's right-panel toggle
        takes over — so there is never a duplicate right-panel icon.
      */}
      {!state.panelOpen && (
        <div className={css.toggleCluster} data-dsh-toggle-cluster>
      ...
      */}
      ...
      >
        ...
        <div className={css.panelBody}>
          ...
          />
          {ctx.get('betterSidebar') !== undefined && (
            <Rail
              ...
              onToggleLeftSidebar={() => { ctx.get('layout')?.toggleSidebar() }}
              leftToggleLabel={t('toggleLeftSidebar')}
            />
          )}
        </div>
        {/*

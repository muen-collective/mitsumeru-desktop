...
export function updateMessage(status: UpdateStatus, locale: UpdateLocale): string {
  ...
  switch (status.phase) {
    ...
    case 'available':
      return zh
        ? `发现新版本${version}，是否更新？`
        : `Mitsumeru${version} is available. Update now?`
    case 'downloading': {
      const percent = Math.round(status.percent ?? 0)
      return zh ? `正在下载更新 ${percent}%` : `Downloading update ${percent}%`
    }
    case 'downloaded':
      return zh ? `Mitsumeru${version} 已下载完成` : `Mitsumeru${version} is ready to install`
    case 'up-to-date':
      return zh ? 'Mitsumeru 已是最新版本' : 'Mitsumeru is up to date'
    case 'unsupported':
